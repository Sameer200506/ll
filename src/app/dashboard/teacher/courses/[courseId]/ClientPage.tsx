...
export default function CourseDetailPage({ params }: { params: Promise<{ courseId: string }> }) {
  ...
  const [lessonForm, setLessonForm] = useState({ title: "", url: "", description: "", pdfUrl: "" });
  const [addingLesson, setAddingLesson] = useState(false);

  // lesson inline edit
  const [editingLessonId, setEditingLessonId] = useState<string | null>(null);
  const [editLessonForm, setEditLessonForm] = useState({ title: "", url: "", description: "", pdfUrl: "" });
  ...
  const handleAddLesson = async (e: React.FormEvent) => {
    ...
    try {
      await addLesson({ courseId, title: lessonForm.title, youtubeUrl: lessonForm.url, description: lessonForm.description, pdfUrl: lessonForm.pdfUrl || "", order: lessons.length });
      toast.success("Lesson added!");
      setLessonOpen(false);
      setLessonForm({ title: "", url: "", description: "", pdfUrl: "" });
  ...
  const handleSaveLesson = async (lessonId: string) => {
    ...
    try {
      await updateLesson(lessonId, { title: editLessonForm.title, youtubeUrl: editLessonForm.url, description: editLessonForm.description, pdfUrl: editLessonForm.pdfUrl || "" });
                      ...
                      >
                        {isEditing ? (
                          <div className="p-4 space-y-3">
                            ...
                            <div className="space-y-1.5">
                              <Label>Lesson PDF URL (Optional)</Label>
                              <Input value={editLessonForm.pdfUrl} onChange={(e) => setEditLessonForm({ ...editLessonForm, pdfUrl: e.target.value })} />
                            </div>
